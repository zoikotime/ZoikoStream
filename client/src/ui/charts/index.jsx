...
// Conversion funnel. `stages` = [{ stage, count, percent, note }].
// Lib-free: recharts has no funnel in this version, and a funnel is a stack of divs whose widths
// are a ratio — a charting library for that is more code, not less.
// A null count is a stage we could not measure (see analytics' `_funnel`); it renders the reason
// instead of a zero-width bar, because an empty bar reads as "nobody got this far".
export function Funnel({ title, subtitle, stages = [], color = "#7c3aed", loading = false }) {
  const top = Math.max(...stages.map((s) => s.count ?? 0), 1);
  return (
    <ChartCard title={title} subtitle={subtitle} loading={loading} empty={!stages.length} height={220}>
      <ol className="space-y-3">
        {stages.map((s, i) => {
          const measured = s.count !== null && s.count !== undefined;
          const width = measured ? Math.max(2, (s.count / top) * 100) : 0;
          const dropped = i > 0 && measured && stages[i - 1].count > s.count
            ? stages[i - 1].count - s.count
            : 0;
          return (
            <li key={s.stage}>
              <div className="mb-1 flex items-baseline justify-between gap-2 text-sm">
                <span className="truncate text-slate-700 dark:text-slate-200">{s.stage}</span>
                <span className="shrink-0 tabular-nums font-medium text-slate-800 dark:text-slate-100">
                  {measured ? s.count.toLocaleString() : "—"}
                  {measured && s.percent != null && (
                    <span className="ml-1.5 font-normal text-slate-400">{s.percent}%</span>
                  )}
                </span>
              </div>
              {measured ? (
                <div className="h-7 w-full overflow-hidden rounded-lg bg-slate-100 dark:bg-slate-800">
                  <div
                    className="flex h-full items-center justify-end rounded-lg pr-2 transition-all"
                    style={{ width: `${width}%`, background: color, opacity: 1 - i * 0.15 }}
                  >
                    {dropped > 0 && width > 22 && (
                ...
                <p className="rounded-lg bg-slate-50 px-2.5 py-1.5 text-xs text-slate-500 dark:bg-slate-800/60 dark:text-slate-400">
                  {s.note || "Not measured"}
                </p>
              )}
            </li>
...
// Weekday × hour audience heat map. `cells` = [{ weekday, hour, viewers }], weekday 0 = Monday.
// A CSS grid rather than a chart library: 7×24 coloured squares is exactly what a grid is for, and
// recharts would need a scatter chart with custom shapes to fake it.
export function HeatMap({ title, subtitle, cells = [], weekdays, color = "#7c3aed", loading = false }) {
  const days = weekdays || ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  const lookup = new Map(cells.map((c) => [`${c.weekday}-${c.hour}`, c.viewers]));
  const peak = Math.max(...cells.map((c) => c.viewers || 0), 1);
  return (
    <ChartCard title={title} subtitle={subtitle} loading={loading} empty={!cells.length}
      emptyText="No audience samples in this range" height={220}>
      <div className="overflow-x-auto">
        <div className="min-w-[560px]">
          <div className="mb-1 grid grid-cols-[2.5rem_repeat(24,minmax(0,1fr))] gap-px">
            <span />
            {Array.from({ length: 24 }, (_, h) => (
              <span key={h} className="text-center text-[9px] tabular-nums text-slate-400">
                {h % 3 === 0 ? h : ""}
              </span>
            ))}
          </div>
          {days.map((day, d) => (
            <div key={day} className="grid grid-cols-[2.5rem_repeat(24,minmax(0,1fr))] items-center gap-px">
              <span className="pr-1 text-right text-[10px] text-slate-500 dark:text-slate-400">{day}</span>
              {Array.from({ length: 24 }, (_, h) => {
                const value = lookup.get(`${d}-${h}`);
                // Undefined (never sampled) and 0 (sampled, nobody watching) are different facts,
                // so an unsampled cell is left blank rather than shaded as the lowest value.
                const intensity = value === undefined ? null : value / peak;
                return (
                  <div
                    key={h}
                    title={value === undefined ? `${day} ${h}:00 — no samples`
                      : `${day} ${h}:00 — ${value.toLocaleString()} viewers`}
                    className={`aspect-square rounded-[2px] ${
                      intensity === null ? "bg-slate-50 dark:bg-slate-800/40" : ""}`}
                    style={intensity === null ? undefined : {
                      background: color,
                      opacity: 0.14 + intensity * 0.86,
                    }}
                  />
                );
              })}
          ...
          <div className="mt-3 flex items-center justify-end gap-1.5 text-[10px] text-slate-400">
            ...
            <span>More · peak {peak.toLocaleString()}</span>
          </div>
        </div>
      </div>
    </ChartCard>
