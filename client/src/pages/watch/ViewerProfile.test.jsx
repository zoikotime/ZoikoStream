...
//
// ── THE TWO THINGS ─────────────────────────────────────────────────────────────────────
//   device profile     name + email, ONE key for the whole browser (zk_viewer_profile).
...
const nameField = () => screen.getByPlaceholderText(/jane doe/i);
const emailField = () => screen.getByPlaceholderText(/jane@company\.com/i);
...
const awaitGate = () => screen.findByRole("heading", { name: /registration required/i });

async function registerFresh(u, { remember = true, name = NAME, email = EMAIL } = {}) {
  await u.type(nameField(), name);
  await u.type(emailField(), email);
  if (remember) await u.click(rememberBox());
  await u.click(screen.getByRole("button", { name: /^register$/i }));
}
...
describe("registering for the first event", () => {
  it("saves the name and email when Remember me is ticked", async () => {
    ...
    await waitFor(() => expect(savedProfile()).toEqual({ name: NAME, email: EMAIL }));
  });
  ...
  it("stores only a name and an email — never a token", async () => {
    const u = userEvent.setup();
    renderWatch(EVENT_A);
    await awaitGate();
    ...
    expect(Object.keys(savedProfile()).sort()).toEqual(["email", "name"]);
...
describe("opening a second event on the same device", () => {
  beforeEach(() => {
    localStorage.setItem(PROFILE_KEY, JSON.stringify({ name: NAME, email: EMAIL }));
  });

  it("offers Continue as {name} with the saved email, not an empty form", async () => {
    renderWatch(EVENT_B);
    await awaitGate();

    expect(screen.getByText(`Continue as ${NAME}`)).toBeInTheDocument();
    expect(screen.getByText(EMAIL)).toBeInTheDocument();
    expect(screen.queryByPlaceholderText(/jane doe/i)).not.toBeInTheDocument();
  });
...
describe("what Continue sends", () => {
  ...
  it("makes exactly one registration request, for the current event", async () => {
    ...
    expect(postsTo(EVENT_B)[0][1]).toEqual({ name: NAME, email: EMAIL });
...
describe("Not you? Change details", () => {
  ...
  it("registers the NEW details and remembers those instead", async () => {
    ...
    await u.clear(emailField());
    await u.type(emailField(), "asha@example.com");
    await u.click(screen.getByRole("button", { name: /^register$/i }));

    await waitFor(() => expect(gate()).not.toBeInTheDocument());
    expect(postsTo(EVENT_B)).toHaveLength(1);
    expect(postsTo(EVENT_B)[0][1]).toEqual({ name: "Asha", email: "asha@example.com" });
    expect(savedProfile()).toEqual({ name: "Asha", email: "asha@example.com" });
  });
  ...
  it("keeps the existing validation", async () => {
    ...
    await u.click(changeDetails());
    await u.clear(emailField());
    await u.type(emailField(), "not-an-email");
...
describe("a device that remembers nobody", () => {
  it("shows the normal name and email form", async () => {
  ...
  it("ignores a stored profile whose email is not an email", async () => {
