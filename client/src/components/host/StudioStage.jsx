      ...
      >
        ...
        <div className="pointer-events-none absolute inset-0 z-[2] grid place-items-center p-6 text-center">
          {screenShare ? null : mediaError ? (
            ...
            <div className="flex max-w-sm flex-col items-center gap-3">
              {/* Concentric rings give the state a centre of gravity on a large black stage,
                  and the outer one breathes so the empty stage doesn't read as a dead feed. */}
              <span className="relative grid h-12 w-12 place-items-center">
                <span
                  aria-hidden="true"
                  className="zk-pulse-ring absolute inset-0 rounded-full bg-violet-500/25"
                />
                <span className="relative grid h-12 w-12 place-items-center rounded-full bg-white/[0.04] ring-1 ring-white/10">
              ...
              {onStartPreview && (
                <button
                  ...
                  className={cx(
                    ...
                    "active:scale-[0.97] active:shadow-md motion-reduce:active:scale-100",
                    "transition-[background-image,box-shadow,transform] duration-200 ease-out motion-reduce:transition-none",
                    focusOnStage
                  )}
                >
                  <FiPlay
                    aria-hidden="true"
                    className="transition-transform duration-200 group-hover:scale-125 motion-reduce:transition-none motion-reduce:group-hover:scale-100"
                  />
                  Start preview
                </button>
              )}
