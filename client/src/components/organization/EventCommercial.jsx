...
// ── Payment dialog: authorizes only — capture/settlement is a Zoiko Finance action (see
// crud.commercial.capture_payment), so the state after this stays "pending" until staff
// settles it. That is the real, current behavior, not a UI bug. ────────────────────────
function PaymentDialog({ onClose, onConfirm, busy, outstanding, currency }) {
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button size="sm" leftIcon={FiCreditCard} loading={busy} disabled={busy || !amount || Number(amount) <= 0}
            onClick={() => onConfirm(Number(amount))}>
            Authorize {money(amount, currency)}
          </Button>
        </>
      }
    >
      <p className="mb-3 text-xs text-amber-600 dark:text-amber-400">
        No real payment processor is connected yet — this simulates authorization only. It
        will show as "Pending" until Zoiko Finance settles it.
      </p>
      <Label variant="console" htmlFor="zk-pay-amount">Amount ({currency})</Label>
      <input
        id="zk-pay-amount" type="number" min="0.01" step="0.01" value={amount}
        onChange={(e) => setAmount(e.target.value)}
        className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-violet-400 focus:ring-2 focus:ring-violet-500/20 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
      />
      {outstanding > 0 && (
...
export default function EventCommercial({ event }) {
  ...
  const payNow = async (amount) => {
    const res = await mutate.run(
      () =>
        api.post(`/commercial/orders/${order.id}/payments/authorize`, {
          amount, idempotency_key: crypto.randomUUID(), provider_name: "mock",
        }),
      { success: "Payment authorized — pending settlement" }
    );
    if (res) setPayOpen(false);
  };
    ...
    <div className="space-y-6">
      ...
      {order && (
        <SectionCard
          ...
          action={
            outstanding > 0 && (
              <Button size="sm" leftIcon={FiCreditCard} onClick={() => setPayOpen(true)}>Make a payment</Button>
            )
          }
        >
          {schedule.length > 0 && (
