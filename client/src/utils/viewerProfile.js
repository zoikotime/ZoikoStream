// The remembered viewer profile: a name and an email, for this browser, for convenience.
//
// ── WHAT THIS IS NOT ────────────────────────────────────────────────────────────────────
// This is deliberately NOT an access credential, and the two must never be confused:
//
//   device profile (here)          name + email. One key for the whole browser. Grants
 ...
 * cookies blocked), or what is stored no longer looks like a name and an email. A bad
 * record reads as "no profile" rather than throwing — a corrupt key must not be able to
 * take the registration form down with it.
 *
...
export function readViewerProfile() {
  try {
    ...
    if (!name.trim() || !isEmail(email)) return null;
 ...
 */
export function saveViewerProfile({ name, email }) {
